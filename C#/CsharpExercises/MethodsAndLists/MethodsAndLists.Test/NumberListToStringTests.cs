...
namespace MethodsAndLists.Test
{
    ...
    public class NumberListToStringTests
    {
        ...
        [TestMethod]
        public void ReportFirstAndLastValue_Linq()
        {
            // Returnera en text som talar om vilket det första och sista numret är i listan
            string result = x.ReportFirstAndLastValue_Linq(new List<int> { 5, 1000, 2000, 3000, 6 });
            Assert.AreEqual("Första siffran är 5 och sista siffran är 6", result);
        ...
        [TestMethod]
        public void ReportNumberOfNegativeValues_Linq()
        {
            // Returnera en text hur många negativa tal som finns i listan
            Assert.AreEqual("Det finns 2 st negativa tal i listan", x.ReportNumberOfNegativeValues_Linq(new List<int> { 5, 7, -2, 100, -4 }));
            Assert.AreEqual("Jippi! Det finns inga negativa tal i listan", x.ReportNumberOfNegativeValues_Linq(new List<int> { 5, 7, 44, 100, 33 }));
            Assert.AreEqual("Det finns ett negativt tal i listan", x.ReportNumberOfNegativeValues_Linq(new List<int> { 5, 7, -2, 100, 4 }));
        }
